...
define(['ojs/ojbufferingdataprovider'], function (BufferingDataProvider) {
  ...
  */
  ...
  var originalRowValues = {};

  PageModule.prototype.areDifferent = function (rowKey, newValue, oldValue) {
    const originalValue = originalRowValues[rowKey];
    if (!originalValue) {
      originalRowValues[rowKey] = oldValue;
    } else {
      const same = JSON.stringify(originalValue) === JSON.stringify(newValue);
      if (same) {
        bufferingDP.resetUnsubmittedItem(rowKey);
        return false;
      }
    }
  ...
  PageModule.prototype.resetOriginalRowValues = function () {
    originalRowValues = {};
  }
